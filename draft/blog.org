...
(中略)
 ...
 #+end_src
** TODO org modeのノードのレベルに応じたインデントを止める        :org:emacs:
:PROPERTIES:
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :archives '(\"2021\" \"2021-07\")
:EXPORT_FILE_NAME: 202107301104
:END:

#+begin_src emacs-lisp
(custom-set-variables '(org-adapt-indentation nil))
#+end_src
* 2022
...
:PROPERTIES:
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :archives '(\"2022\" \"2022-01\")
:EXPORT_FILE_NAME: 202201242348
:END:

最近、戦略周りを考えることがあり、戦略とはなんぞやって本を数冊読んでみた。
本の選択基準は、年末のKindleセール対象になってたとかそれぐらい。

*** 良い戦略、悪い戦略
** DONE ECS上のSpringBootのPrometheusメトリクスをCloudWatchで収集する
