+++
title = "CRI-O + kata-container + Weavenetでkubernetesをインストールする"
date = 2020-07-12T09:33:00+09:00
lastmod = 2020-07-12T09:33:29+09:00
tags = ["kubernetes"]
draft = false
...
特にこれまでkata-containerはさわったことなかったので。
OSはUbuntuを適当に入れた


## kata-containerのインストール {#kata-containerのインストール}
...
OCIとして、runcを使うPodとkata containerを使うPodをデプロイしてみる
...
うまいことnode1とnode2に分散してPodを動かしたので、通常のruncで動くパターンとkata containerで動くパターンのプロセス構成などを見てみる。


#### kata-runtime list {#kata-runtime-list}

kata containerで動いているコンテナのリストは、 `kata-runtime list` で確認することができる。
...
kata containerの場合は、hello-appは直接ホストから見えず、kata-shimで隠蔽されていて、
隔離された環境で動いていることがわかる。


### まとめ {#まとめ}

kata containerは、これまで安全にコンテナ実行するために使う、ぐらいしか聞いておらず
